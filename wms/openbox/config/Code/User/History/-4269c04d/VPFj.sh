#!/bin/bash

wms_path='/home/hoan/dc/wms'

clear
if [[ ! $USER == "root" ]]; then
	echo
	echo -e "Este script não pode ser executado como \e[33;1mroot\e[m"
	echo
	exit 1
fi

# Desenha linha horizontal no tty...
cols=$(tput cols)
printf -v line "%${cols}s" " "
line=${line// /-}

# Função de saída do script...
sair_do_script() {
		echo -e '\nSaindo...\n'
		exit 0
}

# Função para confirmações...
confirmar() {
    read -p "Confirme (s/N)? " sn
    [[ ${sn,,} != "s" ]] && sair_do_script
    echo ""
}

# Função para continuar...
continuar() {
    read -p "Continuar (s/N)? " sn
    [[ ${sn,,} != "s" ]] && return
}

sudo_exist=$(which sudo)
if [ ! -f $sudo_exist ];then
	echo ''
	echo 'Comando [sudo] necessário.'
	echo -e "\nLog como [root] e rode os comandos abaixo:\n"
	echo -e "apt install -y sudo\necho '$USER    ALL=(ALL:ALL) ALL' >> /etc/sudoers\n" | nl
	echo "Após a execução, entre novamente como [$USER] e rode novamente a instalação."
	echo ''
fi

echo $line
echo -e '----- Preparando o sistema instalando pacotes essenciais...' 
echo $line
sleep 3
sudo apt install software-properties-common git vim build-essential curl
sudo apt-add-repository non-free
sudo apt-add-repository contrib
sudo apt update

clear
echo $line
echo -e '----- Pacotes essenciais instalados com sucesso!'
echo $line
sleep 3
clear

echo 'Qual WM deseja instalar?'
echo ''

# Detectar ambientes disponíveis...
ambs=($(ls $wms_path/))
ge=(${ambs[*]} Sair...)

PS3="Digite o número da sua opção ou '${#ge[*]}' para sair: "
select wms in ${ge[*]}; do
    [[ $REPLY -eq ${#ge[*]} ]] && sair_do_script || break
done

clear

# rotina de instalação

echo ''
echo $line
echo ''

echo $wms


